...
function CustomRenderer(eventBus, styles) {
  ...
  this.handlers = {
    'custom:batman': function(p, element) {
      return self.drawBatman(p, element.width);
    },
    'custom:hska': function(p, element) {
      return self.drawHska(p, element.width);
    },
  ...
  this.drawBatman = function(p, side, attrs) {
    var url = require('./batman').dataURL;
    return p.image(url, 0, 0, side*1, side*0.45);
  };

  this.drawHska = function(p, side, attrs) {
    var url = require('./cat').dataURL;
    return p.image(url, 0, 0, side, side);
  };
...
CustomRenderer.prototype.drawBatman = function(visuals, element) {
  var type = element.type;
  var h = this.handlers[type];
  return h(visuals, element);
};

CustomRenderer.prototype.drawHska = function(visuals, element) {
  var type = element.type;
  var h = this.handlers[type];
  return h(visuals, element);
};
...
CustomRenderer.prototype.getShapePath = function(element) {
  ...
  var shapes = {
    ...
    batman: function() { return []; },
    hska: function() { return []; }
  };
